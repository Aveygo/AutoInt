...
pub struct Watchdog {
    clusters: Mutex<Result<Vec<EventCluster>, ()>>,
...
impl Watchdog {
    pub fn new() -> Self {
        ...
        Self {
            clusters: Mutex::new(Err(())),

            model:RustPotion::new(PotionModel::BASE2M, Path::new("models")),
    ...
    fn calc_priorities(&self, embeddings:&Vec<Vec<f32>>, positive_tag:&Vec<f32>, negative_tag:&Vec<f32>) -> Vec<f32> {
        let mut result = vec![];

        for embedding in embeddings {
            let p = self.cosine_similarity(embedding, positive_tag).unwrap();
            let n = self.cosine_similarity(embedding, negative_tag).unwrap();
    ...
    pub fn start(&mut self) {
        ...
        let positive_tag = self.model.encode("High priority news: geopolitics, war, disaster, collapse, presidents, nations");
        let negative_tag = self.model.encode("Low priority news: opinions, celebrity, sports, gossip, local crimes, weather, music, puzzles");
